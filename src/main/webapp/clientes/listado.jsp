    ...
    <div class="bg-white rounded-lg shadow overflow-x-auto">
        <table class="w-full whitespace-nowrap">
            ...
            <tr class="hover:bg-gray-50 transition cliente-row"
                data-cuit="<%= cli.getCuitCuil().toLowerCase() %>"
                data-razon="<%= cli.getRazonSocial().toLowerCase() %>"
                data-mail="<%= cli.getMail().toLowerCase() %>">
...
const tableRows = document.querySelectorAll('.cliente-row'); // Selector simple

function filterTable() {
    const searchText = searchInput.value.toLowerCase();
    
    tableRows.forEach(row => {
        // Usamos los atributos de datos para el filtrado, tal como en el código de usuarios
        const cuil = row.getAttribute('data-cuit') || '';
        const razon = row.getAttribute('data-razon') || '';
...
// --- CÓDIGO DE MENSAJES FLOTANTES (Mantenido) ---
